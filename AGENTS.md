...
- **TypeScript 7 needs `experimental.useTypeScriptCli`**: TS 7 is the native compiler and does not expose the JavaScript compiler API Next uses for its build-time type check, so `next build` fails without that flag in `next.config.mjs`. The flag makes Next shell out to the TypeScript CLI instead. It still gates the build — verified by injecting `const x: number = 'string'` into a file under `src/` and confirming a non-zero exit, a `TS2322`, and no `out/` produced. Re-run that probe whenever the flag changes **or `typescript` takes a major bump** — the Dependabot 6.0.3 → 7.0.2 bump is the case that actually arises, and it re-verified clean. A green CI run is not evidence here: if the check silently stopped running, every build would still pass, which is worse than an old compiler
...
- **`vitest` and `@vitest/*` move together**: the companion packages declare an _exact_ peer on the runner — `@vitest/coverage-v8@5.0.0` peers `vitest@"5.0.0"`, not `^5.0.0` — so a single-package bump cannot resolve at all. It fails in `npm ci` with `ERESOLVE`, before type-check, lint or tests ever run, which makes it look like infrastructure breakage rather than a dependency conflict. `.github/dependabot.yml` has a `vitest` group so the pair is proposed as one PR; keep any future `@vitest/*` addition inside that group rather than adding a second entry
...
- **`overrides` exist to reach security patches upstream pins away from**: `next` pins `postcss` to an exact version and declares `sharp` as an optional `^0.34.x`, so neither can reach its patched release through normal resolution — `package.json` `overrides` lift them. Re-check these on every `next` upgrade: once Next ships a range that already includes the fix, drop the override rather than leaving it to pin something backwards. The postcss override was verified inert by building before and after and diffing the emitted CSS byte-for-byte. `sharp` is never imported here (`images.unoptimized` plus static export means Next's optimizer never runs), so it is patched rather than exercised. `js-yaml` (via `gray-matter`, which parses every post's frontmatter) and `nanoid` (via `postcss`) are the same shape: both advisories are fixed inside the major line their parent already depends on, so the override is a floor rather than a jump — drop each one once the parent's own range reaches the patch. Note that `npm audit` is not wired into CI, so a transitive advisory surfaces only when someone runs it by hand or Dependabot opens a PR. **Editing `overrides` is a two-file change**: npm resolves overrides at install time, so `package-lock.json` must be regenerated in the same commit or `npm ci` refuses the tree outright (`Invalid: lock file's js-yaml@3.14.2 does not satisfy js-yaml@3.15.2`) and every job fails at install. `npm install` alone does not prove this — it rewrites the lock; run `npm ci` to check what CI will actually see. The same applies when porting an upstream commit by hand: upstream's lockfile changes are part of the change, not noise to skip
- **Node baseline lives in three places**: `[tools] node` in `mise.toml` (development plus the install/quality/test jobs — this repo uses mise, not the `.nvmrc` upstream has), `engines.node` in `package.json` (what forks are told they can use), and the `build` matrix in `.github/workflows/node.js.yml` (literals — a matrix cannot read `mise.toml`). The deployed leg is named once as the workflow-level `DEPLOY_NODE` env so the Pages-configure and artifact-upload gates cannot drift apart; a build step asserts `DEPLOY_NODE` still matches the pin via `mise config get tools.node`, which reads the file and so is immune to the `MISE_NODE_VERSION` override the matrix sets. Derive the `engines` floor from the tightest transitive requirement rather than guessing, and re-derive it on every bump of that dependency — `jsdom` is currently the binding one at `^22.22.2 || ^24.15.0 || >=26.0.0` (`@testing-library/jest-dom` wants only `>=22`, which is why Node 20 had to go). `engines.node` is that constraint intersected with the Node lines this fork actually builds, hence `^24.15.0 || >=26.0.0` rather than jsdom's literal value — do not copy upstream's string, which still carries a `^22` arm this fork dropped. **CI cannot catch drift here**: a `24` matrix leg resolves to the newest 24.x, so it satisfies a floor the manifest gets wrong, and the mismatch surfaces only as an `EBADENGINE` warning during `npm ci` that no gate reads
- **`MISE_NODE_VERSION` must be job-level in the build matrix**: as a step-level `env:` it only reaches `mise-action`, and every later `mise run` re-reads `mise.toml` — which silently built all legs on the pinned version and made the matrix a no-op. Nothing asserts this — the job-level `env:` is the entire mechanism, and a green run proves nothing about which version it ran. After touching the matrix, read `mise-action`'s own resolution line in each leg's setup step (`node 24.18.0 MISE_NODE_VERSION=24 24`) and confirm the legs disagree
- **Never commit `mise.lock`** — it is gitignored, and that is deliberate. `mise-action` appends `--locked` automatically whenever a lockfile exists and exposes **no input to disable it** (checked against its `action.yml`), `--locked` demands a lockfile entry for the exact resolved version, and a lockfile holds only one version per tool (`mise lock` replaces the entry rather than appending). The build matrix overrides Node to 24, which can therefore never be in a lock generated from `node = "26"`, so every 24 leg dies in setup with `No lockfile URL found for node@24.21.0 ... (--locked mode)` before a single step runs. mise's own docs call a version-varying matrix the sharpest case of this. A hand-spliced two-entry lock does satisfy `--locked`, but `mise lock` flattens it back to one entry, so it is a hand-maintained file that breaks CI the next time anyone regenerates it. Note `mise install` alone will **not** reproduce the failure locally — only `mise install --locked` will, and only with a tool version that is absent from the lock
- **The shared `node_modules` cache is a fast path, not a dependency**: `install` owns the save; `code-quality` and `test` use `actions/cache/restore` and fall back to `npm ci` when `cache-hit` is not `'true'`. Without that fallback an evicted or not-yet-saved entry leaves them with no `node_modules` and they fail as "command not found" rather than installing — and because they restore on the exact key only, a stale prefix hit can never be mistaken for a current one. Restore-only also stops parallel jobs racing to write the same key
- **mise tasks are aliases, not a second task definition**: `package.json` is the single source of truth for what each command does, and CI calls `npm` directly rather than going through mise. Only add a mise task when it earns a short alias (`mise r b`) or does something npm does not (`ci`, `clean`); a task that is just `npm run <same-name>` is duplication that drifts
...
- **Disabling a section**: set `enabled: false` on its entry in `src/data/routes.ts`. The default export is filtered there, so `Navigation`, `Hamburger` and `Footer` need no changes — those three filters have drifted from each other before, and a fourth condition in each is how it happens again. Disabling has three coupled consequences, all already wired to the flag: the menus drop it, `app/sitemap.ts` drops it _and its children_ by URL prefix, and `createPageMetadata` emits `robots: noindex`. That last one is not optional — `verify-export` enforces indexable ⟺ present in sitemap, so dropping a page from the sitemap while it stays indexable fails the build. `isDisabledPath` therefore matches descendants as well as the section index, and routes that build their own metadata rather than calling `createPageMetadata` — `app/writing/[slug]/page.tsx` — have to call it themselves: publishing a post under a still-disabled `/writing` otherwise exports an indexable page the sitemap omits. The menus cannot derive this from content, because `Navigation` and `Hamburger` are `'use client'` and `routes.ts` must stay free of `fs`; flipping `enabled` is the manual step that launches a section
- **Empty states are derived, not flagged**: the homepage writing section is gated on `recentWriting.length > 0` rather than on the route flag, so it returns by itself when a post is published. Prefer deriving from the data over a second switch to remember
- **Tests must not depend on content that may not exist**: anything reading `content/writing/`, `src/data/writing.ts` or `src/data/projects.ts` injects it with `vi.mock` plus `vi.hoisted` state, so the suite passes on an empty repository and still means something on a full one. Mock `@/lib/posts` (or `@/lib/writing`) rather than writing fixture Markdown. Note `vi.mock` is hoisted above `const`, so shared values must come from `vi.hoisted`; and use plain static imports after the mock — a top-level `await import()` type-checks as TS1378 under this tsconfig even though Vitest runs it fine
- **About copy links are markdown**: `src/data/about.ts` uses `[text](url)`, matching upstream. The `target`/`rel` attributes an outbound link needs are applied once by the `a` override in `src/components/About/Sections.tsx` — pass `MARKDOWN_OPTIONS` to every `<Markdown>` there, or that section's links silently lose `rel="noopener"`. Hash and relative links are deliberately left untouched so section anchors keep working
- **Link a destination once, at first mention**: the about copy linked `loxon.eu` five times in one page, which both Google's developer style guide ("don't provide duplicate links to the same destination") and Wikipedia's MOS:REPEATLINK ("link a term at most once per major section, at first occurrence") advise against. Link the first occurrence and leave later mentions as plain text
- **Editing an about section changes two tests**: `src/lib/__tests__/anchors.test.ts` pins the full list of section titles and their generated ids, and `src/data/__tests__/about.test.ts` asserts on literal substrings of the copy — case-sensitively. Adding, renaming or reordering a `#` heading fails the first; rewording a bullet can fail the second. Both are cheap to update; prefer updating them over bending the copy to fit an assertion
- **Travel entries need a `In YYYY, ` opener**: `Travel / Geography` renders through `extractLogMarker`, which lifts a leading `In 2016, ` or `At 7, ` into the gutter. An entry written any other way keeps its full text and an empty gutter — legal, but it breaks the column, so keep the pattern
- **Facts about the person live in `src/data/profile.json`**: countries visited, city, birth date, role. Tests must read them (`COUNTRIES_VISITED` from `src/lib/telemetry.ts`) rather than repeat the literal — two tests pinned `10` and both had to be edited when the count moved, which is the same drift `countSourceLines()` exists to prevent. Note `birthDate` needs an explicit UTC offset (`+02:00`, Budapest in June) or the stats readout shifts with the visitor's timezone
