...
- Style with CSS custom properties in `src/app/styles/` (tokens in `tokens/`, components in `components/`)
...
- **Profile copy**: Keep role/bio updates in sync across `src/components/Template/Hero.tsx`, `src/app/layout.tsx` metadata, `src/data/about.ts`, and `src/data/resume/work.ts` so homepage copy, SEO, schema, and resume stay aligned
...
- **Fonts must be variable and self-hosted**: `src/app/fonts.ts` feeds versioned Fontsource variable files to `next/font/local`, preserving metric-adjusted fallbacks and critical preloads without a Google build dependency. The weight tokens only render as distinct weights because those files remain variable. Newsreader italic is scoped through `src/app/writing/layout.tsx` and deliberately is not preloaded site-wide
...
- **Opting out of base link styles**: `src/app/styles/base/links.css` paints an accent underline on every `p a` / `li a`. Navigation-like links must set `background-image: none` or they pick up a rule that appears nowhere else — this is what made the mobile menu look broken
- **Print**: `src/app/styles/print.css` is imported last so it can override both themes. The resume is the page people print; check it there after changing resume layout
